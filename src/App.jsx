...
// ── Constants ────────────────────────────────────────
const VCE_COORDS = [17.391, 78.44]; // Vasavi College of Engineering
...
for (const bus of BUS_DATA) {
  const overrideStops = ROUTE_COORDINATE_OVERRIDES[bus.num];
  if (!overrideStops) {
    continue;
  }

  const lastKnownTime = bus.stops[bus.stops.length - 1]?.time || "8:00 AM";
  bus.stops = overrideStops.map((stop, index) => ({
    name: stop.name,
    coords: [stop.lat, stop.lng],
    time: bus.stops[index]?.time || lastKnownTime,
  }));
}

const BACKEND_BUS_INDEX = Object.fromEntries(BUS_DATA.map((bus, index) => [bus.num, `BUS_${index + 1}`]));
function getBackendBusIdForNum(busNum) {
  return BACKEND_BUS_INDEX[busNum] || "BUS_1";
}

function isLiveBusLikelyOnRoute(bus, liveBus) {
  if (!liveBus || !Number.isFinite(liveBus.lat) || !Number.isFinite(liveBus.lng)) {
    return false;
  }

  const speed = Number(liveBus.speed);
  if (!Number.isFinite(speed) || speed < 3 || speed > 90) {
    return false;
  }

  const ts = Date.parse(liveBus.timestamp || "");
  ...
  const nearestStopDistanceKm = Math.min(
    ...bus.stops.map((stop) => haversineKm(liveBus.lat, liveBus.lng, stop.coords[0], stop.coords[1])),
  );

  return nearestStopDistanceKm <= 0.45;
}

// ── Helpers ──────────────────────────────────────────
const walkMins = distKm => Math.ceil((distKm / WALK_SPEED_KMH) * 60);

function getFallbackBusSpeedKmh(bus) {
  return bus.status === "delayed" ? 20 : 30;
}

function estimateEtaMinutesForPoints(fromCoords, toCoords, speedKmh) {
  const distKm = haversineKm(fromCoords[0], fromCoords[1], toCoords[0], toCoords[1]);
  return Math.max(1, Math.round((distKm / Math.max(speedKmh, 8)) * 60));
}
...
function buildGoogleMapsRouteUrl(routeBus) {
  if (!routeBus?.stops?.length) {
    return "https://www.google.com/maps";
  }

  const origin = routeBus.stops[0].coords;
  const destination = routeBus.stops[routeBus.stops.length - 1].coords;
  const waypoints = routeBus.stops
    .slice(1, -1)
...
function getAlternates(missed, sLat, sLng, preferredStopName) {
  const MAX_BOARDING_DISTANCE_KM = 3.5;
  const preferredKey = normalizeStopName(preferredStopName);
  const ranked = BUS_DATA
    .filter(b => b.num !== missed.num && b.status !== "offline")
    .map(bus => {
      const { stop, distKm, idx } = getNearestStop(bus, sLat, sLng);
      const isDestinationStop = idx >= bus.stops.length - 1 || stop.name.toLowerCase().includes("vce");
      if (isDestinationStop) {
        return null;
      }

      if (distKm > MAX_BOARDING_DISTANCE_KM) {
        return null;
      }

      const simT = getSimulatedBusProgress(bus);
      const progressIdx = simT * (bus.stops.length - 1);
      const hasRecentlyPassedStop = progressIdx > idx + 0.25;
      if (hasRecentlyPassedStop) {
        return null;
      }

      const simPos = getBusPositionAtT(bus, simT);
      const wm = walkMins(distKm);
      const speedKmh = getFallbackBusSpeedKmh(bus);
      const busEta = estimateEtaMinutesForPoints(simPos, stop.coords, speedKmh);
      const canCatch = wm <= busEta + 4;
  ...
  if (ranked.length >= 2 || !preferredKey) {
    return ranked;
  }

  const used = new Set(ranked.map((item) => item.bus.num));
  const areaCandidates = BUS_DATA
    .filter(b => b.num !== missed.num && b.status !== "offline" && !used.has(b.num))
    .map((bus) => {
      const matchingIndices = bus.stops
        .map((stop, idx) => ({ idx, key: normalizeStopName(stop.name), stop }))
        .filter(({ key, idx }) => idx < bus.stops.length - 1 && (key.includes(preferredKey) || preferredKey.includes(key)))
        .map(({ idx }) => idx);
      ...
      const simT = getSimulatedBusProgress(bus);
      const progressIdx = simT * (bus.stops.length - 1);
      const targetIdx = matchingIndices.find((idx) => idx >= progressIdx - 0.2) ?? matchingIndices[0];
      const stop = bus.stops[targetIdx];
      const simPos = getBusPositionAtT(bus, simT);
      const distKm = haversineKm(sLat, sLng, stop.coords[0], stop.coords[1]);
      const walkMinsLocal = walkMins(distKm);
      const speedKmh = getFallbackBusSpeedKmh(bus);
      const busEta = estimateEtaMinutesForPoints(simPos, stop.coords, speedKmh);
      ...
      return {
        ...
        idx: targetIdx,
        distKm,
        walkMins: walkMinsLocal,
        busEta,
        canCatch: walkMinsLocal <= busEta + 4,
        simulated: false,
        score: distKm * 0.7 + busEta * 0.45,
      };
    })
    .filter(Boolean)
    .sort((a, b) => a.score - b.score)
  ...
  return [...ranked, ...areaCandidates].slice(0, 3);
}
...
function BarcodeScanner({ onScan, onClose }) {
  ...
  const FALLBACK_ROLL = "1602-23-733-174";

  const handleDecodedText = (rawText) => {
    if (!rawText || scannedOnceRef.current) return;

    const text = String(rawText).trim();
    const rollMatch = text.match(/1602[-–\s]\d{2}[-–\s]\d{3}[-–\s]\d{1,3}/);
    if (rollMatch) {
      scannedOnceRef.current = true;
      onScan(rollMatch[0].replace(/[–\s]/g, "-").replace(/--+/g, "-"));
    } else {
  ...
  useEffect(() => {
    ...
    const onHotkey = (event) => {
      ...
      const isRequestedCombo = key === "i" && event.ctrlKey && event.metaKey;
      const isBrowserFriendlyFallback = key === "i" && event.ctrlKey && event.altKey;

      if (!isRequestedCombo && !isBrowserFriendlyFallback) {
        return;
      }

      event.preventDefault();
      setStatus(`Fallback roll injected: ${FALLBACK_ROLL}`);
      onScan(FALLBACK_ROLL);
    };
    ...
    async function startScanner() {
      try {
        ...
        setError("");
        setStatus("Point camera at your college ID barcode (good lighting helps)");
        setScanning(true);

        const tryNativeBarcodeDetector = async () => {
          if (!("BarcodeDetector" in window) || nativeScanTimerRef.current) return false;

          try {
            const supported = await window.BarcodeDetector.getSupportedFormats();
            const preferredFormats = ["code_128", "code_39", "ean_13", "qr_code", "pdf417"];
            ...
            nativeScanTimerRef.current = window.setInterval(async () => {
              if (!active || !videoRef.current || scannedOnceRef.current) return;
              try {
                ...
                // Keep polling while camera is active.
              }
            }, 250);
        ...
        const startZxing = () => {
          ...
          try {
            ...
            reader.decodeFromVideoDevice(undefined, videoRef.current, (result) => {
              if (!active || scannedOnceRef.current) return;
              if (result?.getText) {
        ...
        let zxingStarted = startZxing();
        if (!zxingStarted) {
          const zxingRetryTimer = window.setInterval(() => {
            if (!active || scannedOnceRef.current) {
              window.clearInterval(zxingRetryTimer);
              return;
            }
            zxingStarted = startZxing();
            if (zxingStarted) {
              window.clearInterval(zxingRetryTimer);
            }
          }, 700);
        ...
        // Native detector fallback helps on some laptop webcams.
        const nativeStarted = await tryNativeBarcodeDetector();
        if (!zxingStarted && !nativeStarted) {
          setStatus("Camera active, but scanner library not ready. Use manual input or retry.");
        }
      } catch (err) {
        if (!active) return;
        setError("Camera could not start. Allow camera permission, close other camera apps, and try again.");
        setScanning(false);
        setStatus("");
      }
    }
      ...
      <div style={{width:"min(420px,94vw)",display:"flex",flexDirection:"column",gap:16,animation:"slideUp .35s ease"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            ...
            <div style={{fontSize:12,color:"var(--muted)",marginTop:2}}>Point camera at barcode on your VCE ID. Hotkey fallback: Ctrl+Win+I or Ctrl+Alt+I</div>
          </div>
...
// ONBOARDING
// ══════════════════════════════════════════════════════
function Onboarding({ onLogin, onSwitchAdmin }) {
      ...
      <div style={{position:"fixed",inset:0,zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",background:"radial-gradient(ellipse at 30% 20%,rgba(0,229,255,.08),transparent 60%),radial-gradient(ellipse at 70% 80%,rgba(123,97,255,.08),transparent 60%),var(--bg)"}}>
        <div style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:24,padding:"48px 44px",width:"min(500px,94vw)",boxShadow:"0 32px 80px rgba(0,0,0,.6)",position:"relative",overflow:"hidden",animation:"slideUp .4s ease"}}>
          ...
          <div style={{marginTop:16,textAlign:"center",fontSize:12,color:"var(--muted)"}}>
            Are you an admin?{" "}
            <button onClick={onSwitchAdmin} style={{background:"none",border:"none",color:"var(--accent)",cursor:"pointer",fontWeight:700,fontSize:12}}>Admin login</button>
          </div>
        </div>
      </div>
    </>
...
// ADMIN LOGIN
// ══════════════════════════════════════════════════════
function AdminLogin({ onLogin, onSwitchStudent }) {
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      if (adminId.toLowerCase() === "admin" && password === "vce@2026") {
        onLogin({ adminId });
        return;
      }
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",background:"radial-gradient(ellipse at 20% 20%,rgba(255,201,74,.08),transparent 60%),radial-gradient(ellipse at 80% 80%,rgba(0,229,255,.06),transparent 60%),var(--bg)"}}>
      <div style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:24,padding:"44px 42px",width:"min(460px,94vw)",boxShadow:"0 32px 80px rgba(0,0,0,.6)",position:"relative",overflow:"hidden",animation:"slideUp .4s ease"}}>
        <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:"linear-gradient(90deg,var(--gold),var(--accent),var(--accent2))"}}/>

        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:26}}>
          <div style={{width:48,height:48,background:"linear-gradient(135deg,var(--gold),var(--accent))",borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>🛡️</div>
          <div style={{fontFamily:"Syne,sans-serif"}}>
            <strong style={{display:"block",fontSize:20,fontWeight:800,letterSpacing:"-.5px"}}>Admin Control Hub</strong>
            <span style={{fontSize:11,color:"var(--muted)",letterSpacing:2,textTransform:"uppercase"}}>Transport Operations</span>
          </div>
        </div>

        <div style={{marginBottom:16}}>
          <label style={{display:"block",fontSize:11,letterSpacing:"1.5px",textTransform:"uppercase",color:"var(--muted)",marginBottom:8,fontWeight:500}}>Admin ID</label>
          <input value={adminId} onChange={e=>setAdminId(e.target.value)} placeholder="admin" style={inp}/>
        </div>

        <div style={{marginBottom:10}}>
          <label style={{display:"block",fontSize:11,letterSpacing:"1.5px",textTransform:"uppercase",color:"var(--muted)",marginBottom:8,fontWeight:500}}>Password</label>
          <input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="vce@2026" style={inp} onKeyDown={e => e.key === "Enter" && submit()}/>
          <div style={{fontSize:11,color:"var(--muted)",marginTop:6,opacity:.7}}>Demo: admin / vce@2026</div>
        </div>

        <button onClick={submit} disabled={loading} style={{width:"100%",padding:14,background:loading?"rgba(255,255,255,.06)":"linear-gradient(135deg,var(--gold),var(--accent))",border:"none",borderRadius:12,color:"#fff",fontFamily:"Syne,sans-serif",fontSize:14,fontWeight:700,cursor:loading?"default":"pointer",letterSpacing:".4px",marginTop:12,display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
          {loading ? <><span style={{width:16,height:16,border:"2px solid rgba(255,255,255,.3)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin 1s linear infinite",display:"inline-block"}}/> Verifying...</> : "Enter Admin Hub →"}
        </button>
        {error && <div style={{color:"var(--red)",fontSize:13,marginTop:12,lineHeight:1.5}}>⚠️ {error}</div>}

        <div style={{marginTop:16,textAlign:"center",fontSize:12,color:"var(--muted)"}}>
          Student login?{" "}
          <button onClick={onSwitchStudent} style={{background:"none",border:"none",color:"var(--accent)",cursor:"pointer",fontWeight:700,fontSize:12}}>Go back</button>
        </div>
      </div>
    </div>
  );
...
// MISSED BUS MODAL
// ══════════════════════════════════════════════════════
function MissedBusModal({ bus, sLat, sLng, preferredStopName, onDismiss }) {
  const alts = useMemo(() => getAlternates(bus, sLat, sLng, preferredStopName), [bus.num, sLat, sLng, preferredStopName]);
  return (
        ...
        <div style={{padding:"20px 24px 28px",display:"flex",flexDirection:"column",gap:12}}>
          ...
          {alts.length === 0 ? (
            <div style={{background:"rgba(255,201,74,.07)",border:"1px solid rgba(255,201,74,.2)",borderRadius:12,padding:16,fontSize:14,color:"var(--gold)",textAlign:"center"}}>
              No nearby buses are crossing your current area right now. Call transport: <strong>+91-40-2351-0177</strong>
            </div>
          ) : alts.map((alt, i) => (
            <div key={alt.bus.num} style={{background:alt.canCatch?"rgba(0,230,118,.05)":"var(--card)",border:`1px solid ${alt.canCatch?"rgba(0,230,118,.3)":"var(--border)"}`,borderRadius:16,padding:"16px 18px",position:"relative",overflow:"hidden",animation:`slideInRight .3s ease ${i*0.1}s both`}}>
              ...
              {alt.simulated && (
                <div style={{fontSize:10,color:"var(--accent2)",marginBottom:8,letterSpacing:1,textTransform:"uppercase"}}>Simulation fallback</div>
              )}
              ...
              <button
                onClick={() => window.open(buildGoogleMapsRouteUrl(alt.bus), "_blank", "noopener,noreferrer")}
                style={{width:"100%",marginBottom:alt.canCatch?10:0,padding:"9px 12px",background:"rgba(0,229,255,.08)",border:"1px solid rgba(0,229,255,.25)",borderRadius:10,color:"var(--accent)",fontFamily:"Syne,sans-serif",fontSize:12,fontWeight:700,cursor:"pointer"}}
              >
                Select Option to See Route of This Bus
              </button>
              {alt.canCatch && (
...
function StudentDashboard({ roll }) {
  ...
  const [liveBusesById, setLiveBusesById] = useState({});
  const [backendConnected, setBackendConnected] = useState(false);
  const animRef = useRef(null);
  const alarmFired = useRef(false);
  const missedFired = useRef(false);
  const missedAutoTimer = useRef(null);
  const speedJitter = useRef(0);
  const forcedStopInitialized = useRef(false);

  useEffect(() => { const c=()=>{ if(window.L) setLeafletOk(true); else setTimeout(c,300);}; c(); },[]);

  useEffect(() => {
    if (!("Notification" in window)) {
      setNotifInfo("This browser does not support notifications.");
      return;
    }

    if (Notification.permission === "granted") {
      setNotifGranted(true);
      setNotifInfo("Alerts are enabled.");
    } else if (Notification.permission === "denied") {
      setNotifGranted(false);
      setNotifInfo("Notifications are blocked in browser settings.");
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const hydrateBuses = async () => {
      try {
        const response = await fetch(`${BACKEND_URL}/api/buses`);
        if (!response.ok) return;
        const data = await response.json();
        if (!mounted || !Array.isArray(data?.buses)) return;

        setLiveBusesById((prev) => {
          const next = { ...prev };
          data.buses.forEach((item) => {
            if (item?.bus_id) next[item.bus_id] = item;
          });
          return next;
        });
    ...
    socket.on("bus-location", (payload) => {
      ...
      setLiveBusesById((prev) => ({ ...prev, [payload.bus_id]: payload }));
    });
  ...
  useEffect(() => {
    if (!forceMehdipatnamStop || forcedStopInitialized.current) {
      return;
    }

    const idx = bus.stops.findIndex((stop) => stop.name.toLowerCase().includes("mehdipatnam") || stop.name.toLowerCase().includes("mhdipatnam"));
    if (idx < 0 || bus.stops.length < 2) {
      return;
    }

    const targetT = Math.max(0.02, Math.min(0.96, (idx / (bus.stops.length - 1)) - 0.06));
    setBusT(targetT);
    forcedStopInitialized.current = true;
  }, [bus, forceMehdipatnamStop]);
  ...
  const requestNotif = async () => {
    ...
    if (!window.isSecureContext && window.location.hostname !== "localhost" && window.location.hostname !== "127.0.0.1") {
      setNotifInfo("Notifications require HTTPS or localhost.");
      return;
    }
    ...
    try {
      ...
      if (granted) {
        setNotifInfo("Alerts enabled successfully.");
      } else if (permission === "denied") {
        setNotifInfo("Permission denied. Enable notifications in site settings.");
      } else {
        setNotifInfo("Notification permission was not granted.");
      }
    } catch {
      setNotifInfo("Unable to enable notifications right now.");
    }
  };

  // ── Realistic simulation: 22-minute full cycle, variable speed ──
  useEffect(() => {
    // 22 min cycle → ~1320 seconds → each frame ~16ms
    // We want busT to go 0→1 in ~1320 * 60fps = 79200 frames
    // Increment per frame: 1/79200 ≈ 0.0000126
    // But we also simulate stops (bus pauses ~3-5s at each stop)
    const CYCLE_FRAMES = 79200; // 22 min at 60fps
    ...
    const go = () => {
      setBusT(t => {
        ...
        // Traffic jitter: random speed variation ±20%
        const jitter = 0.8 + Math.random() * 0.4;
        const inc = BASE_INC * jitter * (bus.status === "delayed" ? 0.75 : 1);
        const n = t + inc;
        return n >= 1 ? 0.02 : n;
      });
      ...
      if (speedJitter.current === 0) {
        const baseSpeed = bus.status === "delayed" ? 18 : 28;
        setSpeed(Math.max(0, baseSpeed + Math.floor(Math.random() * 16 - 6)));
      }
  ...
  const liveBusCandidate = liveBusesById[backendBusId] || null;
  const liveBus = isLiveBusLikelyOnRoute(bus, liveBusCandidate) ? liveBusCandidate : null;
  const busPos = liveBus ? [liveBus.lat, liveBus.lng] : getBusPositionAtT(bus, busT);
  const mehdipatnamIdx = forceMehdipatnamStop
    ? bus.stops.findIndex((stop) => stop.name.toLowerCase().includes("mehdipatnam") || stop.name.toLowerCase().includes("mhdipatnam"))
    : -1;
  const forcedStop = mehdipatnamIdx >= 0 ? bus.stops[mehdipatnamIdx] : null;

  const { stop: myStop, idx: myStopIdx, distKm: distToStop } = forceMehdipatnamStop && forcedStop
    ? {
      stop: forcedStop,
      idx: mehdipatnamIdx,
      distKm: sLat ? haversineKm(sLat, sLng, forcedStop.coords[0], forcedStop.coords[1]) : 0.3,
    }
    : sLat
  ...
  const liveSpeed = liveBus?.speed;
  const effectiveSpeed = Number.isFinite(liveSpeed) ? liveSpeed : speed;
  const busEta = Number.isFinite(liveBus?.eta_minutes)
    ? liveBus.eta_minutes
    : Math.max(1, Math.round((busDistToMyStop / Math.max(effectiveSpeed, 10)) * 60));
  ...
  const cannotCatchCurrentBus = locGranted && leaveNow && busEta < walkTime;

  // Alarm trigger disabled - user requested to remove automatic 0 min notification on login
  // useEffect(() => {
  //   if (leaveIn <= 5 && !alarmFired.current && !alarmDone && locGranted) {
  //     alarmFired.current = true;
  //     setShowAlarm(true);
  //
  //     if (notifGranted) {
  //       const body = leaveIn === 0
  //         ? `You should leave now for ${myStop.name}.`
  //         : `Leave in ${leaveIn} min for ${myStop.name}!`;
  //       new Notification(`🚌 Bus B${bus.num}`, { body });
  //     }
  //   }
  //   if (leaveIn > 5) alarmFired.current = false;
  // }, [leaveIn, locGranted, alarmDone, notifGranted, bus.num, myStop?.name]);
  ...
  useEffect(() => {
    if (cannotCatchCurrentBus && !missedFired.current && !showMissed) {
      if (!missedAutoTimer.current) {
        missedAutoTimer.current = setTimeout(() => {
          missedAutoTimer.current = null;
          if (!missedFired.current) {
            missedFired.current = true;
            setShowMissed(true);
          }
        }, 60000);
      }
    ...
    if (missedAutoTimer.current) {
      clearTimeout(missedAutoTimer.current);
      missedAutoTimer.current = null;
    }
  }, [cannotCatchCurrentBus, showMissed]);
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)",paddingBottom:76}}>
      ...
      <div style={{padding:tab === "map" ? 0 : "18px 18px 0"}}>
        ...
        {tab === "home" && (
          <div style={{display:"flex",flexDirection:"column",gap:14,animation:"fadeIn .3s ease"}}>
            ...
            {locGranted && notifInfo && (
              <div style={{fontSize:12,color:"var(--muted)"}}>{notifInfo}</div>
            )}
        ...
        {tab === "map" && (
          <div style={{animation:"fadeIn .3s ease"}}>
            <div style={{borderRadius:0,overflow:"hidden",border:"none",height:"calc(100vh - 58px - 56px)",position:"relative"}}>
              {leafletOk ? (
                <LiveMap
                  bus={bus}
                  studentLocation={sLat && sLng ? { lat: sLat, lng: sLng } : null}
                  nearestStop={myStop}
                  busProgress={busT}
                  liveBusPosition={liveBus ? [liveBus.lat, liveBus.lng] : null}
                  liveMeta={liveBus}
                />
              ) : (
        ...
        {tab === "stops" && (
          <div style={{animation:"fadeIn .3s ease"}}>
            ...
            {bus.stops.map((stop, i) => {
              ...
              const etaMins = estimateEtaMinutesForPoints(busPos, stop.coords, effectiveSpeed);
              return (
      ...
      {showMissed && (
        <MissedBusModal
          bus={bus}
          sLat={sLat || bus.stops[0].coords[0]}
          sLng={sLng || bus.stops[0].coords[1]}
          preferredStopName={myStop?.name}
          onDismiss={()=>{setShowMissed(false);missedFired.current=false;}}
        />
      )}
    </div>
  );
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)",paddingBottom:32}}>
      <div style={{position:"sticky",top:0,zIndex:50,background:"rgba(8,12,20,.96)",backdropFilter:"blur(20px)",borderBottom:"1px solid var(--border)",padding:"0 18px",display:"flex",alignItems:"center",justifyContent:"space-between",height:58}}>
        ...
        <button onClick={onBack} style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:9,padding:"5px 13px",color:"var(--muted)",fontSize:11,cursor:"pointer",fontFamily:"DM Sans,sans-serif"}}>← Logout</button>
      </div>
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)",color:"var(--text)"}}>
      <Particles/>
      {!studentRoll && !adminSession && authMode === "student" && (
        <Onboarding
          onLogin={(roll) => { setStudentRoll(roll); setView("student"); }}
          onSwitchAdmin={() => setAuthMode("admin")}
        />
      )}
      {!studentRoll && !adminSession && authMode === "admin" && (
        <AdminLogin
          onLogin={(admin) => { setAdminSession(admin); setView("admin"); }}
          onSwitchStudent={() => setAuthMode("student")}
        />
      )}
      {studentRoll && view === "student" && <StudentDashboard roll={studentRoll} />}
      {adminSession && view === "admin" && (
        <AdminDashboard onBack={() => { setAdminSession(null); setAuthMode("admin"); setView("student"); }} />
      )}
    </div>
  );
}
